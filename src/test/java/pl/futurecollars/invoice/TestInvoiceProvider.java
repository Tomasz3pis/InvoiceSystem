...
public class TestInvoiceProvider {
    ...
    public Company getGBCCompany() {
        return new Company(7896541234L, "St steerra", "GBC");
    }

    public Company getMijagiCompany() {
        return new Company(9081233113L, "Uuki st", "Mijagi");
    }

    public Company getUnomiCompany() {
        return new Company(9080081223L, "Bonsa st", "Unomi");
    }

    public InvoiceEntry getFirstEntry() {
        return new InvoiceEntry("do this", new BigDecimal(1500), Vat.VAT_23, new BigDecimal(5));
    }

    public InvoiceEntry getSecondEntry() {
        return new InvoiceEntry("do that", new BigDecimal(600), Vat.VAT_8, new BigDecimal(2));
    }

    public InvoiceEntry getThirdEntry() {
        return new InvoiceEntry("also this", new BigDecimal(650), Vat.VAT_5, new BigDecimal(3));
    }

    public InvoiceEntry getFourthEntry() {
        return new InvoiceEntry("something", new BigDecimal(1100), Vat.VAT_ZW, new BigDecimal(1));
    }

    public InvoiceEntry getFifthEntry() {
        return new InvoiceEntry("anything", new BigDecimal(100), Vat.VAT_0, new BigDecimal(4));
    }

    public Invoice getInvoiceWith5Entries() {
        return new InvoiceBuilder()
                .withSeller(getGBCCompany())
                .withBuyer(getUnomiCompany())
